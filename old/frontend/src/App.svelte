<script>
  ...
  async function handleSubmit() {
    ...
    if (tourneyData && tourneyData.divisions) {
      ...
      player1Stats = null;
      player2Stats = null;
      standings = []; // Clear standings when a new file is loaded
    }
  }
  ...
  // Function to load selections from the backend
  async function loadSelections() {
    try {
      ...
      if (data) {
        jsUrl = data.tourneyUrl || "http://localhost:8080/tourney.js";
        if (jsUrl !== null) { handleSubmit(); }
        selectedDivision = data.selectedDivision;
        if(selectedDivision !== null) { handleDivisionChange(); }
