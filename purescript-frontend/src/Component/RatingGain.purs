-- | RatingGain Overlay Component
-- | Displays tournament leaders sorted by rating gain
...
render state =
  if state.loading then
    ...
    Nothing -> case state.tournament of
      Just tournament -> renderRatingGain state
      Nothing -> BaseOverlay.renderError "No tournament data"

renderRatingGain :: forall m. State -> H.ComponentHTML Action () m
renderRatingGain state =
                      ...
                      [ HH.table
                          ...
                          [ HH.thead_
                              [ HH.tr
                                  ...
                                  , HH.th [ HP.class_ (HH.ClassName $ "px-4 py-1 text-center " <> theme.colors.textPrimary <> " text-xl font-black uppercase tracking-wider") ] [ HH.text "Rating +/-" ]
                                  , HH.th [ HP.class_ (HH.ClassName $ "px-4 py-1 text-center " <> theme.colors.textPrimary <> " text-xl font-black uppercase tracking-wider") ] [ HH.text "New Rating" ]
                                  , HH.th [ HP.class_ (HH.ClassName $ "px-4 py-1 text-center " <> theme.colors.textPrimary <> " text-xl font-black uppercase tracking-wider") ] [ HH.text "Old Rating" ]
                                  ]
                              ]
                          , HH.tbody_
...
renderPlayerRow theme index player =
  ...
  in
    HH.tr
      ...
      , HH.td
          [ HP.class_ (HH.ClassName $ "px-4 py-1 text-center text-xl font-bold") ]
          [ HH.span
              [ HP.class_ (HH.ClassName $ "font-mono font-black text-2xl " <> ratingGainColor) ]
      ...
      , HH.td
          ...
          [ HH.span
              [ HP.class_ (HH.ClassName "font-mono font-black text-2xl") ]
              [ HH.text $ show player.initialRating ]
          ]
      ]
