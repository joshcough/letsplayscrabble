...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.raw('GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO scrabble_user');
  await knex.raw('GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA public TO scrabble_user');
}

export async function down(knex: Knex): Promise<void> {
  // Drop triggers first
  await knex.raw('DROP TRIGGER IF EXISTS update_tournaments_updated_at ON tournaments');
  await knex.raw('DROP TRIGGER IF EXISTS update_current_matches_updated_at ON current_matches');
