...
type SourceType =
  ...
  | "player1-under-cam-no-seed" | "player2-under-cam-no-seed"
  | "player1-under-cam-small" | "player2-under-cam-small"
  | "player1-under-cam-with-rating" | "player2-under-cam-with-rating"
  | "player1-points" | "player2-points"
  | "player1-game-history" | "player2-game-history"
  | "player1-game-history-small" | "player2-game-history-small"
  | "player1-bo7" | "player2-bo7"
  | "tournament-data";

// Helper function to get recent games for a player and convert to GameResult format
const getRecentGamesForPlayer = (playerId: number, games: DB.GameRow[], players: DB.PlayerRow[], limit: number = 5) => {
  // Filter games where this player participated
  const playerGames = games.filter(game =>
    game.player1_id === playerId || game.player2_id === playerId
  );

  // Sort by round number (descending) to get most recent games
  const sortedGames = playerGames.sort((a, b) => b.round_number - a.round_number);
...
const MiscOverlay: React.FC = () => {
  ...
  if (matchLoading || !currentMatch) {
    return <div className="text-black p-2">Loading current match...</div>;
  }
  ...
  return (
    <BaseOverlay>
      {({ tournament, divisionData, divisionName }) => {
        ...
        const { calculateStandingsFromGames } = require("../../utils/calculateStandings");
        ...
        if (!currentGame) {
          return <div className="text-black p-2">Current game not found in tournament data</div>;
        }

        // Find the two players from current game
        const player1Stats = rankedPlayers.find((p: any) => p.playerId === currentGame.player1_id);
        const player2Stats = rankedPlayers.find((p: any) => p.playerId === currentGame.player2_id);

        if (!player1Stats || !player2Stats) {
          return <div className="text-black p-2">Players not found in tournament data</div>;
        }
        ...
        const renderPlayerData = (source: SourceType) => {
          ...
          const playerId = isPlayer1 ? currentGame.player1_id : currentGame.player2_id;
          ...
          switch (source) {
            ...
            case "player2-game-history":
              const recentGames = getRecentGamesForPlayer(playerId, divisionData.games, divisionData.players);
              return (
                <div>
            ...
            case "player2-game-history-small":
              const recentGamesSmall = getRecentGamesForPlayer(playerId, divisionData.games, divisionData.players);
              return (
                <GameHistoryDisplay
                  games={recentGamesSmall}
                  side={side}
                />
              );
