  ...
  const overlays = [
    {
      title: "Worker Page",
      path: "/worker",
      description: "Background service that manages real-time data updates for all overlays",
      isSpecial: true
    },
    {
      title: "Misc Overlay Testing",
      path: "/overlay/misc_testing",
      description: "Test page showing all available misc overlay sources and examples"
    },
    {
  ...
  return (
    <div className="container mx-auto p-8">
        ...
        {overlays.map((overlay) => (
          ...
          >
            ...
            <div className={`mt-4 text-sm ${
              ...
              {overlay.isSpecial ? "Add as Browser Source →" : "Add as Browser Source →"}
            </div>
          </Link>
        ))}
      </div>
      ...
      <div className="mt-8 p-4 bg-orange-50 border border-orange-200 rounded-lg">
        <h3 className="font-semibold text-orange-800 mb-2">⚠️ Important - Worker Browser Source Required:</h3>
        <ul className="text-orange-700 text-sm space-y-1">
          <li>• <strong>Must add the Worker Page as a Browser Source</strong> in OBS for real-time updates to work</li>
          <li>• <strong>The worker handles:</strong> WebSocket connections, tournament polling, and data broadcasting</li>
          <li>• <strong>All overlay Browser Sources depend on the worker</strong> - without it, data won't update automatically</li>
          <li>• <strong>One worker per user</strong> - only add one worker Browser Source to avoid conflicts</li>
          <li>• <strong>Keep worker Browser Source enabled</strong> even if not visible in your scene</li>
        </ul>
      </div>
      ...
      <div className="mt-6 p-4 bg-[#FFF8E7] border border-[#4A3728]/20 rounded-lg">
        ...
        <ul className="text-[#6B5744] text-sm space-y-1">
          ...
          <li>• <strong>Example:</strong> /users/{userId}/overlay/standings/123/A shows standings for tournament 123, division A</li>
          <li>• <strong>Misc overlay:</strong> Add ?source=player1-name (or other sources) for specific data elements</li>
        </ul>
      </div>
      ...
      <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <ul className="text-blue-700 text-sm space-y-1">
          <li>• <strong>Step 1:</strong> Add Worker Page as a Browser Source (can be in any scene, even if not visible)</li>
          <li>• <strong>Step 2:</strong> Add your overlay Browser Sources using the URLs below</li>
          <li>• <strong>All Browser Sources are scoped to your user account</strong> (ID: {userId})</li>
          <li>• <strong>Worker must stay enabled</strong> for live data updates across all overlays</li>
        </ul>
      </div>
    </div>
  );
