...
export interface TournamentService {
  ...
  getTournament(userId: number, tournamentId: number): Promise<Domain.Tournament>;
  getTournamentSummary(userId: number, tournamentId: number): Promise<Domain.TournamentSummary>;
  ...
  // Division operations
  getDivisions(userId: number, tournamentId: number): Promise<Domain.Division[]>;
  getPlayersForDivision(userId: number, tournamentId: number, divisionName: string): Promise<Domain.Player[]>;
  
  // Polling operations
  enablePolling(tournamentId: number, request: StartPollingRequest): Promise<PollingSuccessData>;
...
export interface CurrentMatchService {
  ...
  setCurrentMatch(request: Domain.CreateCurrentMatch): Promise<Domain.CurrentMatch>;
}
...
export interface ApiService extends AuthService, TournamentService, CurrentMatchService {
