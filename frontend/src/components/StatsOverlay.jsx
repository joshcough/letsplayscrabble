...
const StatsOverlay = () => {
  ...
  useEffect(() => {
    ...
    socket.on('matchUpdate', (data) => {
      ...
      const orderedPlayers = firstPlayer.id === data.player1_id
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-between p-8 pointer-events-none">
      ...
      <div className="bg-black/50 text-white p-4 rounded-lg w-64">
        <h2 className="text-xl font-bold mb-2">{player1?.name || 'Player 1'}</h2>
      ...
      {/* Right player stats */}
      <div className="bg-black/50 text-white p-4 rounded-lg w-64">
        <h2 className="text-xl font-bold mb-2">{player2?.name || 'Player 2'}</h2>
        <div className="space-y-1">
